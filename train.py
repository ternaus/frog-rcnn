...
def run_train():
    ...
    backup_project_as_zip(PROJECT_PATH, str(out_dir / 'backup' / ' code.train.%s.zip') % IDENTIFIER)
            ...
            for b in range(batch_size):
                ...
                if truth_box is not None:
                    for box, label, instance in zip(truth_box, truth_label, truth_instance):
                        ...
                        image1 = contour[:, :, np.newaxis] * np.array((0, 255, 0)) + (
                                1 - contour[:, :, np.newaxis]) * image1
